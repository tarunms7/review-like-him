...
class TestNarrowExceptionHandling:
    """Tests for narrow exception handling in scan and helper methods."""

    @pytest.mark.asyncio()
    async def test_scan_modules_httpx_error_graceful_degradation(
        self, mock_github_client,
    ) -> None:
        """HTTPStatusError in _detect_modules yields empty modules list."""
        root_contents = [
            _dir_entry("pyproject.toml"),
            _dir_entry("src", "dir"),
        ]
        ...
        async def mock_get_contents(owner, repo, path):
            ...
            if path == "pyproject.toml":
                return {"content": _b64("[project]\nname = 'x'\n")}
            # For _detect_modules (listing directories), raise HTTP error
            if path == "src":
                raise httpx.HTTPStatusError(
                    "Server Error",
                    request=httpx.Request("GET", "https://api.github.com"),
    ...
    async def test_scan_api_contracts_key_error_logged(
        ...
        """KeyError in _detect_api_contracts is logged at WARNING."""
        root_contents = [
            _dir_entry("pyproject.toml"),
            _dir_entry("src", "dir"),
        ]
        ...
        original_detect = scanner._detect_api_contracts

        async def raise_key_error(*args, **kwargs):
            raise KeyError("missing_field")

        scanner._detect_api_contracts = raise_key_error  # type: ignore[assignment]

        import logging
        with caplog.at_level(logging.WARNING, logger="review-bot"):
            ctx = await scanner.scan("owner", "repo")

        assert ctx.api_contracts == []
        assert any(
            "Failed to detect API contracts" in r.message and "missing_field" in r.message
    ...
    async def test_read_file_unicode_error_returns_none(
        ...
        # Create binary content that is valid base64 but not valid UTF-8
        bad_bytes = bytes([0x80, 0x81, 0xFF, 0xFE])
        b64_content = base64.b64encode(bad_bytes).decode("ascii")
    ...
    async def test_load_repo_config_yaml_error_returns_default(
        ...
        expected = RepoConfig.default()
        assert config.version == expected.version
        assert config.min_severity == expected.min_severity
