...
class TestNarrowExceptionHandling:
    """Tests for narrowed exception handling in scan-related methods."""

    @pytest.mark.asyncio()
    async def test_scan_modules_httpx_error_graceful_degradation(
        self, mock_github_client,
    ) -> None:
        """HTTPStatusError in module detection yields empty modules list."""
        root_contents = [
            _dir_entry("src", "dir"),
            _dir_entry("pyproject.toml"),
        ]
        ...
        async def mock_get_contents(owner, repo, path):
            ...
            if path == "pyproject.toml":
                return {"content": _b64("[project]\nname = 'myapp'\n")}
            # Fail on any dir listing (module detection)
            if path == "src":
                raise httpx.HTTPStatusError(
                    "Server Error",
                    request=httpx.Request("GET", "https://api.github.com"),
        ...
        # Module detection failed but scan still returned a valid context
    ...
    async def test_scan_api_contracts_key_error_logged(
        ...
        """KeyError in API contract detection is logged at WARNING."""
        import logging
        ...
        scanner._detect_api_contracts = raise_key_error
    ...
    async def test_read_file_unicode_error_returns_none(
        ...
        # Create content that is valid base64 but invalid UTF-8
        raw_bytes = bytes([0xFF, 0xFE, 0x80, 0x81])
    ...
    async def test_load_repo_config_yaml_error_returns_default(
        ...
        default = RepoConfig.default()
        assert config.min_severity == default.min_severity
